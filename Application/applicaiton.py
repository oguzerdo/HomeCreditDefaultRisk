...
def application_train_test(num_rows=None, nan_as_category=False):
    # Read data and merge
    df = pd.read_csv(r'C:\Users\oe\Desktop\HCDR\data\application_train.csv', nrows=None)
    test_df = pd.read_csv(r'C:\Users\oe\Desktop\HCDR\data\application_test.csv', nrows=None)
    print("Train samples: {}, test samples: {}".format(len(df), len(test_df)))
    df = df.append(test_df).reset_index()
    df = df[df['CODE_GENDER'] != 'XNA']

    # FEATURE ENGINEERING
    # NaN değerleri 365243 olarak girilmiş, onlar düzeltildi
    df['DAYS_EMPLOYED'].replace(365243, np.nan, inplace=True)

    # Araba yaş değeri boş olan gözlemler 0 olarak atandı
    ...
    # FEATURE 1 - MAAŞ / KİŞİ SAYISI YA DA MAAŞ / ÇOCUK SAYISI ( MODEL SONUCUNA GÖRE KARŞILAŞTIR )
    # df['NEW_INC_PER_CHLD'] = df['AMT_INCOME_TOTAL'] / (1 + df['CNT_CHILDREN'])
    df['NEW_INC_PERS'] = df['AMT_INCOME_TOTAL'] / df['CNT_FAM_MEMBERS']

    # FEATURE 2 - Kredi miktarı / aile sayısı
    df['NEW_AMT/FAM'] = df['AMT_CREDIT'] / df['CNT_FAM_MEMBERS']

    # FEATURE 3 - KREDİNİN YILLIK ÖDEMESİ / GELİR ***
    df['NEW_ANNUITY_INCOME_PERC'] = df['AMT_ANNUITY'] / df['AMT_INCOME_TOTAL']

    # FEATURE X - GELİR / YILLIK KREDİ * FAMILYSIZE (ATILABİLİR)
    df["NEW_FAMILY_EFFECT"] = df['NEW_AMT/FAM'] / df['CNT_FAM_MEMBERS']

    # FEATURE X - ALMAK İSTEDİĞİ MAL VE ÇEKTİĞİ KREDİ ARASINDAKİ FARKA GÖRE DERECELENDİRME
    ...
    # df.loc[((df["EXT_SOURCE_1"] < 0.5) | (df["EXT_SOURCE_2"] < 0.55) | (df["EXT_SOURCE_3"] < 0.45)) & (
    #         df["NEW_AMT_STATUS"] == 1), "NEW_BOMB"] = 2

    # FEATURE 9 - DOKUMANLARIN TOPLAMI / DOCS ATILDI
    docs = [f for f in df.columns if 'FLAG_DOC' in f]
    df['DOCUMENT_COUNT'] = df[docs].sum(axis=1)
    df.drop(docs, axis=1, inplace=True)

    # FEATURE 10 - AGE RANK 1: YOUNG 5: OLDER
    ...
    # FEATURE X NEW_PHONE_TO_BIRTH_RATIO
    df['NEW_PHONE_TO_BIRTH_RATIO'] = df['DAYS_LAST_PHONE_CHANGE'] / df['DAYS_BIRTH']
    # FEATURE X NEW_PHONE_TO_BIRTH_RATIO_EMPLOYER
    df['NEW_PHONE_TO_BIRTH_RATIO_EMPLOYER'] = df['DAYS_LAST_PHONE_CHANGE'] / df['DAYS_EMPLOYED']

    # FEATURE X - NEW_INC_ORG : Sektöründeki maaş ortalamaları
    ...
    def cols_n():
        ...
        print(num_cols)
        print("--" * 25)
        print("More than Thresholds Category Columns", len(other_cols))
        print(other_cols)
        print("--" * 25)
